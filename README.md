...
Assim que o `version.json` publicado tiver uma versão maior que a instalada, o Hub mostra o aviso automaticamente (a checagem roda a cada ~12h, e também ao clicar em "Verificar agora"). A URL do `version.json` já vem embutida de fábrica no código (`https://raw.githubusercontent.com/batistela/hub-chamados/main/version.json`) — ninguém precisa configurar nada na primeira instalação. O campo "URL do arquivo de versão" nas Configurações só existe pra quem quiser trocar (ex: apontar pra um fork) ou desativar o aviso (deixando em branco).
