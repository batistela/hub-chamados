...
Assim que o `version.json` publicado tiver uma versão maior que a instalada, o Hub mostra o aviso automaticamente (a checagem roda a cada ~12h, e também ao clicar em "Verificar agora"). A URL do `version.json` (`https://raw.githubusercontent.com/batistela/hub-chamados/main/version.json`) é fixa no código de `background.js` (constante `UPDATE_CHECK_URL`) — não existe campo nas Configurações pra isso, de propósito, pra não poluir a tela com uma opção que praticamente ninguém precisa mexer. Se um dia precisar apontar pra outro repositório (ex: um fork), é só editar essa constante no código.
